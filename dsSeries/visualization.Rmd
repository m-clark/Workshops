...
A word about using plotly. I like what I've been able to do with it, but I don't find it easy to work with once you go beyond your basic sorts of visualizations, and this is mostly because of the poor [documentation](https://plot.ly/r/), which is pretty terrible in both layout and detail. When you can find an interesting example (most stop at the simplest demonstrations), it typically won't contain text or comments outlining the 'why' of the code. In many cases, I've found it easier to look at the python examples or go to the javascript library documentation itself to see better examples and what is possible. Furthermore, the pages load *every* plot, and the plots are not small in web browsing terms, so some pages will take a bit to load even with a decent internet connection (e.g. this document is over 4mb). Unfortunately, aside from a couple examples, the help files just point you to the web for detail, and since there are few working examples for many of the options, it's hard to pinpoint why a plot doesn't do what you want[^plotlyerror].  
