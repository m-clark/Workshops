...
We begin with a discussion on <span class="emph">web scraping</span>. The term itself is ambiguous, and could potentially mean anything[^otherwsnames], but the gist is that there is something out on the web (or some connected machine) that we want, and we'd like ot use R to get it. This section won't (at least for now) get into lower level utilities such as that provided by <span class="pack">httr</span> or <span class="pack">Rcurl</span>, though some packages will be using them under the hood.  Instead focus will be on higher-level approaches with an eye toward common tasks.
...
As an additional example let's get some data from [basketball-reference.com](http://www.basketball-reference.com/).  The following gets the totals table from the url[^bref]. Issues include header rows after every 20th row of data, and converting all but a few columns from character to numeric.
...
A lot of times we'll want to grab text as opposed to tables.  See the [API chapter]() for an example.

#### Images

Images are fairly easy because they are simply files with specific extensions like \*svg \*gif etc.
...
picofday = base_page %>% 
  ...
  # html_nodes('a') %>%
  # html_attr("href") %>% 
  # grep(pattern='jpg', value=T) %>% 
  html_nodes('img') %>%
  html_attr('src') 

download.file(url=paste0('https:', picofday), destfile='img/picofday.jpg', mode='wb')
  # rasterImage()
  # paste0('https://', .)
rasterImage(jpeg::readJPEG(source='img/picofday.jpg'))
plot(2:1, type='n')
rasterImage(jpeg::readJPEG(source='img/picofday.jpg'), 1, 2, 2, 1)

test = jpeg::readJPEG(source='img/picofday.jpg')
qplot(wt, mpg, data=mtcars, geom='blank') + annotation_custom(grid::rasterGrob(test)) + lazerhawk::theme_trueMinimal()
```


## Issues

As mentioned, the ease with which you will be able to scrape a website will depend a lot on how well the page/site is put together. Many are cookie-cutter templates designed with no regard to data availability whatsoever, others are just the result of amateurs that do not do web design for a living, and still others are just poorly designed.
...
[^bs]: I highly recommend [Beautiful Soup](https://www.crummy.com/software/BeautifulSoup/) also. Until <span class="pack">rvest</span> and related packages I preferred using BeautifulSoup to what was available in R.
