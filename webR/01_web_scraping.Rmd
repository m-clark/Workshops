...
Lots of attributes for practically every element.
...
```{r}
html_nodes(towns, '.wikitable.sortable') %>% html_table() %>% .[[1]] %>% head
```

In general though, if you know it's a table, then use html_table.  When the table has no class then you'll have to use a magic number to grab it, so don't expect your code to hold up in the future when the website changes.
...
I won't go into details about the rest of the code, as that's for another [workshop](http://m-clark.github.io/workshops/dplyr/mainSlides.html).
